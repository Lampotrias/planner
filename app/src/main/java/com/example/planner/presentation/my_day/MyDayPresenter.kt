...
class MyDayPresenter @Inject constructor(
    ...
    fun processSaveEvent(event: EventTransferObject) {
        ...
        saveAndReturnEventInteractor(Event(0, event.name, calendar.timeInMillis, TimeZone.getDefault().rawOffset)) {it.fold(this::handleError, this::successSaveEvent)}
    }
    ...
    private fun successGetEventList(list: List<Event>){
        ...
        list.map {
            ...
            eventsModel.add(EventModel(it.id, it.name, format.format(date)))
        }
